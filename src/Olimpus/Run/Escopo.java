...
public class Escopo {
	...
	public boolean run(AST ASTC) {
		...
		if (ASTC.getNome().contentEquals("DEF")) {
			...
			if (divisor != 0) {
				...
				mRunTime.getErros().add("Opera��o Invalida : O Divisor nao pode ser nulo !");
				ret = false;
			}
	...
	public boolean Function_Copiar(String eTypeGet, String eTypeDef) {
		...
		if (duplicado == true) {
			mRunTime.getErros().add("J� existe uma FUNCTION chamada : " + eTypeDef);
			return false;
		} else {
			if (enc_action == true) {
	...
	public boolean Action_Save(String eNome,String eFunctionDO) {
		...
		if (enc == true) {
			mRunTime.getErros().add("Já existe uma action com esse nome : " + eNome);
			ret = false;
		}else{
	...
	public boolean Runner(AST ASTRaiz, String Function_DO, ArrayList<String> withs, Escopo eInternal) {
		...
		if (enc == false) {
			mRunTime.getErros().add("FUNCTION n�o encontrada : " + Function_DO);
			ret = false;
		}
	...
	public boolean Caller(AST ASTRaiz) {
		...
		if (enc == false) {
			mRunTime.getErros().add("ACTION n�o encontrada " + ASTRaiz.getValor());
			ret = false;
		}
